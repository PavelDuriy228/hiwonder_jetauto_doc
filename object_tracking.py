#!/usr/bin/env python3
# encoding: utf-8
# KCF/MedianFlow трекинг объекта с удержанием дистанции.
# Улучшения vs оригинал:
#   - PIDController с anti-windup и D-членом (вместо P-only из jetauto_sdk)
#   - FirstOrderFilter на linear.x — нет рывков при смене дистанции
#   - freeze_integrator в мёртвой зоне дистанции (±10 см)

import cv2
import rospy
import numpy as np
import jetauto_sdk.fps as fps
import jetauto_sdk.pwm_servo as pwm_servo_mod
from sensor_msgs.msg import Image
from geometry_msgs.msg import Twist

PAN_CENTER  = 1500
TILT_CENTER = 1500
PAN_MIN,  PAN_MAX  = 800, 2200
TILT_MIN, TILT_MAX = 800, 2200
PAN_SIGN  = 1
TILT_SIGN = 1

_DT = 1.0 / 30.0   # камера ~30 Hz


class PIDController:
    """PID с anti-windup (интегратор не растёт при насыщении выхода).

    Источник идеи: openpilot/common/pid.py
    """

    def __init__(self, kp, ki, kd, pos_limit, neg_limit):
        self.kp = kp
        self.ki = ki
        self.kd = kd
        self.pos_limit = pos_limit
        self.neg_limit = neg_limit
        self.i = 0.0
        self.last_error = 0.0

    def reset(self):
        self.i = 0.0
        self.last_error = 0.0

    def update(self, error, freeze_integrator=False):
        """Принимает ошибку (setpoint − measured), возвращает управляющий сигнал."""
        p = self.kp * error
        d = self.kd * (error - self.last_error) / _DT
        self.last_error = error
        if not freeze_integrator:
            new_i = self.i + self.ki * _DT * error
            if self.neg_limit < p + new_i + d < self.pos_limit:
                self.i = new_i
        output = p + self.i + d
        return max(self.neg_limit, min(self.pos_limit, output))


class FirstOrderFilter:
    """LP-фильтр первого порядка.

    Источник идеи: openpilot/common/filter_simple.py
    """

    def __init__(self, x0, rc):
        self.alpha = _DT / (rc + _DT)
        self.x = float(x0)

    def update(self, value):
        self.x = (1.0 - self.alpha) * self.x + self.alpha * float(value)
        return self.x


class KCFNode:
    def __init__(self, name):
        rospy.init_node(name)
        self.name = name

        # Дистанционный PID: error = target_cm − measured_cm
        # go_speed компенсирует смещение нуля (исходная логика сохранена)
        self.pid_d    = PIDController(kp=0.8,  ki=0.05, kd=0.02,
                                      pos_limit=100.0, neg_limit=-100.0)
        self.pid_pan  = PIDController(kp=0.5,  ki=0.01, kd=0.01,
                                      pos_limit=200.0, neg_limit=-200.0)
        self.pid_tilt = PIDController(kp=0.5,  ki=0.01, kd=0.01,
                                      pos_limit=200.0, neg_limit=-200.0)

        # Фильтр скорости: rc=0.15 с — убирает рывки при потере/поиске объекта
        self.lin_filter = FirstOrderFilter(x0=0.0, rc=0.15)

        self.go_speed = 0.007
        self.linear_x = 0.0
        self.pan_pos  = PAN_CENTER
        self.tilt_pos = TILT_CENTER
        self.pan_servo  = pwm_servo_mod.PWMServo(1)
        self.tilt_servo = pwm_servo_mod.PWMServo(2)
        self.pan_servo.start()
        self.tilt_servo.start()
        self.pan_servo.set_position(self.pan_pos,  500)
        self.tilt_servo.set_position(self.tilt_pos, 500)

        self.fps = fps.FPS()

        self.DISTANCE  = 100    # целевая дистанция, см
        self.distance  = self.DISTANCE
        self.mouse_click  = False
        self.selection    = None
        self.track_window = None
        self.drag_start   = None
        self.start_circle = True
        self.start_click  = False
        self.depth_frame  = None

        self.tracker = cv2.legacy.TrackerMedianFlow_create()
        fs = cv2.FileStorage("custom_csrt.xml", cv2.FILE_STORAGE_READ)
        fn = fs.getFirstTopLevelNode()
        self.tracker.read(fn)

        cv2.namedWindow(name, 1)
        cv2.setMouseCallback(name, self.onmouse)
        camera = rospy.get_param('/depth_camera/camera_name', 'camera')
        self.image_sub = rospy.Subscriber(
            '/%s/rgb/image_raw' % camera, Image,
            self.image_callback, queue_size=1)
        self.depth_image_sub = rospy.Subscriber(
            '/%s/depth/image_raw' % camera, Image,
            self.depth_image_callback, queue_size=1)
        self.mecanum_pub = rospy.Publisher(
            '/jetauto_controller/cmd_vel', Twist, queue_size=1)
        rospy.sleep(0.2)
        self.mecanum_pub.publish(Twist())

    def depth_image_callback(self, depth_image):
        self.depth_frame = np.ndarray(
            shape=(depth_image.height, depth_image.width),
            dtype=np.uint16, buffer=depth_image.data)

    def image_callback(self, ros_image):
        rgb_image = np.ndarray(
            shape=(ros_image.height, ros_image.width, 3),
            dtype=np.uint8, buffer=ros_image.data)
        try:
            result_image = self.image_proc(np.copy(rgb_image))
        except BaseException as e:
            print(e)
            result_image = rgb_image
        cv2.imshow(self.name, cv2.cvtColor(result_image, cv2.COLOR_RGB2BGR))
        key = cv2.waitKey(1)
        if key != -1:
            self.mecanum_pub.publish(Twist())
            self.pan_servo.set_position(PAN_CENTER,  500)
            self.tilt_servo.set_position(TILT_CENTER, 500)
            rospy.signal_shutdown('shutdown')

    def onmouse(self, event, x, y, flags, param):
        if event == cv2.EVENT_LBUTTONDOWN:
            self.mouse_click = True
            self.drag_start  = (x, y)
            self.track_window = None
        if self.drag_start:
            xmin = min(x, self.drag_start[0])
            ymin = min(y, self.drag_start[1])
            xmax = max(x, self.drag_start[0])
            ymax = max(y, self.drag_start[1])
            self.selection = (xmin, ymin, xmax, ymax)
        if event == cv2.EVENT_LBUTTONUP:
            self.mouse_click  = False
            self.drag_start   = None
            self.track_window = self.selection
            self.selection    = None
            roi = self.depth_frame[
                self.track_window[1]:self.track_window[3],
                self.track_window[0]:self.track_window[2]]
            try:
                roi_h, roi_w = roi.shape
                if roi_h > 0 and roi_w > 0:
                    roi_cut = [1/3, 1/3]
                    if roi_w < 100:
                        roi_cut[1] = 0
                    if roi_h < 100:
                        roi_cut[0] = 0
                    roi_distance = roi[
                        int(roi_h*roi_cut[0]):(roi_h - int(roi_h*roi_cut[0])),
                        int(roi_w*roi_cut[1]):(roi_w - int(roi_w*roi_cut[1]))]
                    self.distance = int(
                        np.mean(roi_distance[
                            np.logical_and(roi_distance > 0,
                                           roi_distance < 30000)]) / 10)
                else:
                    self.distance = self.DISTANCE
            except Exception:
                self.distance = self.DISTANCE
        if event == cv2.EVENT_RBUTTONDOWN:
            self.mouse_click  = False
            self.selection    = None
            self.track_window = None
            self.drag_start   = None
            self.start_circle = True
            self.start_click  = False
            self.mecanum_pub.publish(Twist())
            self.pid_d.reset()
            self.lin_filter   = FirstOrderFilter(x0=0.0, rc=0.15)
            self.tracker = cv2.legacy.TrackerMedianFlow_create()
            fs = cv2.FileStorage("custom_csrt.xml", cv2.FILE_STORAGE_READ)
            fn = fs.getFirstTopLevelNode()
            self.tracker.read(fn)

    def image_proc(self, image):
        if self.start_circle:
            if self.track_window:
                cv2.rectangle(image,
                    (self.track_window[0], self.track_window[1]),
                    (self.track_window[2], self.track_window[3]),
                    (0, 0, 255), 2)
            elif self.selection:
                cv2.rectangle(image,
                    (self.selection[0], self.selection[1]),
                    (self.selection[2], self.selection[3]),
                    (0, 0, 255), 2)
            if self.mouse_click:
                self.start_click = True
            if self.start_click and not self.mouse_click:
                self.start_circle = False
            if not self.start_circle:
                print('start tracking')
                bbox = (
                    self.track_window[0], self.track_window[1],
                    self.track_window[2] - self.track_window[0],
                    self.track_window[3] - self.track_window[1])
                self.tracker.init(image, bbox)
        else:
            twist = Twist()
            ok, bbox = self.tracker.update(image)
            if ok and min(bbox) > 0:
                p1 = (int(bbox[0]), int(bbox[1]))
                p2 = (int(bbox[0] + bbox[2]), int(bbox[1] + bbox[3]))
                center = [(p1[0] + p2[0]) / 2.0, (p1[1] + p2[1]) / 2.0]
                roi = self.depth_frame[p1[1]:p2[1], p1[0]:p2[0]]
                roi_h, roi_w = roi.shape
                if roi_h > 0 and roi_w > 0:
                    roi_cut = [1/3, 1/3]
                    if roi_w < 100:
                        roi_cut[1] = 0
                    if roi_h < 100:
                        roi_cut[0] = 0
                    roi_distance = roi[
                        int(roi_h*roi_cut[0]):(roi_h - int(roi_h*roi_cut[0])),
                        int(roi_w*roi_cut[1]):(roi_w - int(roi_w*roi_cut[1]))]
                    try:
                        distance = int(
                            np.mean(roi_distance[
                                np.logical_and(roi_distance > 0,
                                               roi_distance < 30000)]) / 10)
                    except Exception:
                        distance = self.DISTANCE

                    cv2.putText(image, 'Distance: %dcm' % distance,
                                (10, 460), cv2.FONT_HERSHEY_SIMPLEX,
                                0.6, (255, 255, 0), 1)
                    cv2.rectangle(image, p1, p2, (0, 255, 0), 2, 1)
                    h, w = image.shape[:2]

                    # Дистанционный PID (с anti-windup)
                    in_deadband = abs(distance - self.distance) < 10
                    error_d = self.distance - distance   # >0 = слишком далеко
                    pid_out  = self.pid_d.update(error_d,
                                                 freeze_integrator=in_deadband)
                    raw_tmp  = self.go_speed - pid_out
                    raw_tmp  = max(-0.15, min(0.15, raw_tmp))
                    if abs(raw_tmp) < 0.008:
                        raw_tmp = 0.0
                    # Фильтр сглаживает резкие переходы скорости
                    twist.linear.x = self.lin_filter.update(raw_tmp)

                    # Pan — anti-windup: серво на пределе = не растим I
                    pan_error = w / 2.0 - center[0]
                    at_pan_limit = (self.pan_pos <= PAN_MIN or
                                    self.pan_pos >= PAN_MAX)
                    if abs(center[0] - w / 2.0) > 10:
                        delta_pan = self.pid_pan.update(
                            pan_error, freeze_integrator=at_pan_limit)
                        self.pan_pos = int(
                            self.pan_pos + PAN_SIGN * delta_pan)
                        self.pan_pos = max(PAN_MIN, min(PAN_MAX, self.pan_pos))
                        self.pan_servo.set_position(self.pan_pos, 20)

                    # Tilt
                    tilt_error = h / 2.0 - center[1]
                    at_tilt_limit = (self.tilt_pos <= TILT_MIN or
                                     self.tilt_pos >= TILT_MAX)
                    if abs(center[1] - h / 2.0) > 10:
                        delta_tilt = self.pid_tilt.update(
                            tilt_error, freeze_integrator=at_tilt_limit)
                        self.tilt_pos = int(
                            self.tilt_pos - TILT_SIGN * delta_tilt)
                        self.tilt_pos = max(TILT_MIN, min(TILT_MAX, self.tilt_pos))
                        self.tilt_servo.set_position(self.tilt_pos, 20)
                else:
                    cv2.putText(image, 'Tracking failure detected !',
                                (10, 460), cv2.FONT_HERSHEY_SIMPLEX,
                                0.6, (255, 255, 0), 1)
                    twist.linear.x = self.lin_filter.update(0.0)
            else:
                cv2.putText(image, 'Tracking failure detected !',
                            (10, 460), cv2.FONT_HERSHEY_SIMPLEX,
                            0.6, (255, 255, 0), 1)
                twist.linear.x = self.lin_filter.update(0.0)
            self.mecanum_pub.publish(twist)

        self.fps.update()
        return self.fps.show_fps(image)


def main():
    node = KCFNode('kcf_tracking')
    try:
        rospy.spin()
    except Exception as e:
        node.mecanum_pub.publish(Twist())
        node.pan_servo.set_position(PAN_CENTER,  500)
        node.tilt_servo.set_position(TILT_CENTER, 500)
        rospy.logerr(str(e))


if __name__ == '__main__':
    main()
